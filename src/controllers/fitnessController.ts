...
import {
  FitnessClubModel,
  FitnessDocument,
} from "../database/models/fitnessClub";
...
export class FitnessController {
  public async find(req: Request, res: Response, next: NextFunction) {
    try {
      let query: any = {};
      const result = await FitnessClubModel.find(query);
  ...
  public async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const foundTrainer = await FitnessClubModel.findOne({
        $or: [
          {
            name,
          },
      ...
      const saved = await FitnessClubModel.create({
        name,
        email,
        phoneNumber,
        ...rest,
  ...
  public async findOne(req: Request, res: Response, next: NextFunction) {
    try {
      const found = await FitnessService.find({ _id: req.params.id });
  ...
  public async findByEmail(req: Request, res: Response, next: NextFunction) {
    try {
      const found = await FitnessService.find({ email: req.params.email });
  ...
  public async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const updated = await FitnessClubModel.updateOne(
        { _id },
        { ...req.body }
      );
  ...
  public async requestAddTrainer(
    ...
    try {
      ...
      const foundTrainer: FitnessDocument = await FitnessService.find({
        _id: req.params.id,
      });
  ...
  public async addDisciple(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      let foundTrainer: FitnessDocument = await FitnessService.find({
        _id: req.params.id,
      });

      if (!foundTrainer) {
        throw createHttpError(StatusCodes.NOT_FOUND, "Fitness club not found");
      }
  ...
  public async delete(req: Request, res: Response, next: NextFunction) {
    try {
      await FitnessService.delete(req.params.id);
